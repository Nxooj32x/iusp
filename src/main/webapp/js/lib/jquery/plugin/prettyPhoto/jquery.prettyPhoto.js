...
<a href="#" class="pp_arrow_next">Next</a> \
				...
				switch (_getFileType(pp_images[set_position])) {
					case 'image':
						...
						imgPreloader.onerror = function() {
							alert('图标无法加载，请确定路径是否正确或者图片是否存在！');
							$.prettyPhoto.close();
						};
		...
		function _resize_overlay() {
			//bug(YB1145)：遮罩层初始化的地方已经做了修改(737行)，以修改后的设计宽高的模式，是不需执行该方法中的代码的。
			//       虽然改动适用于PC和移动端的屏幕及浏览器。但插件中的原有代码在PC的浏览器是没有问题的，为了尽量不改动原有插件的原则，
			//       增加了在移动端的流程器上(SAFARi、UC)即触摸屏端，才会按修改后的代码处理，包括737行初始化的修改，也按这样的 
...
//			}
		...
		function _build_overlay(caller) {
			...
			//结合636行处理YBbug，重新设定 遮罩层的的实现机制，下面注释掉的为原始代码
			$pp_overlay.css({
				'opacity': 0,
				'position': "fixed",
				'bottom': "0",
