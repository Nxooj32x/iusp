    ...
    var isMac = w.navigator.platform == 'MacIntel',
        ...
        events = {
            ...
            paste: function(e) {
            	...
            	if ($(this).attr('orgPaste')) {
            		elem.removeAttr('orgPaste');
            		return;
            	}
                ...
                if (tempArea.length < 1) {
                    ...
                    tempArea.attr('id', id);
                    elem.parent().append(tempArea);
                }
                tempArea.focus();

                setTimeout(function() {
                	 var clipboardContent = tempArea.val();
                     tempArea.remove();
                     utils.selection.restore(range);
                     elem.focus();
                     elem.select();
                     if (clipboardContent) {
                     	clipboardContent = SureAjax.xsscheck(clipboardContent);
                     	var ret = d.execCommand('insertHTML', false, clipboardContent);
                     	if (!ret) {
                     		if(window.console){
                     		console.log("doPaste");
                     		}
                     		elem.attr('orgPaste', true);
                     		d.execCommand('Paste', false, clipboardContent);
                     	}
                     	elem.trigger('keyup');
                     } else {
                     	SureMsg.msg("不支持复制该内容");
                     }
                }, 500);
            }
        };
