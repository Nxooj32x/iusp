!(function (window, $) {
    ...
    var __PayApiUtil = {
		...
		buildPayPopHtml : function (config){
			...
			var returnHtml = "";
			//弹出 支付提示
			returnHtml+=' <div id="pay_wait_result" class="pay_tip  hidden">';
			...
			returnHtml+='<div class="wechatPay_box hidden wx_pay_tip" style="text-align:center;">';
		...
		initPopEvent : function(config, sCB){
			$('#pay_wait_result .btn_group .fail').unbind('click');
			$('#pay_wait_result .btn_group .fail').click(function () {
				SureUtil.closeDialog('popDoPayingDialog');
				art.dialog({
				    id: "popDoPayingFailDialog",
				    title: "温馨提示",
				    lock: true,
				    padding: 0,
				    content: document.querySelector('#pay_fail_box'),
				    close: function () {
				    }
				});
				
			});
			$("#pay_fail_box .btn_group .confirm").unbind('click');
			$("#pay_fail_box .btn_group .confirm").click(function () {
		    	SureUtil.closeDialog('popDoPayingFailDialog');
		    });
			

			//遮罩按钮事件
			$('#pay_wait_result .btn_group .success').unbind('click');
			$('#pay_wait_result .btn_group .success').click(function () {
		...
		buildPayFormHtml : function (config) {
			...
			for (var i = 0; i < config.supportPayWay.length; i++) {
				if (config.supportPayWay[i] == SurePayUtil.PAY_TYPE_UPOP) {
				    ...
				    returnHtml += ' <label for="unipay"><img src="' + config.staticUrl + 'img/pay/unionpay.png" width="150" height="45" alt="银联在线支付"></label>';
				    returnHtml += '</li>';
				}
				if (config.supportPayWay[i] == SurePayUtil.PAY_TYPE_ALI) {
				    returnHtml += '<li>';
				    returnHtml += ' <input id="alipay" type="radio" name="formPayWayR"  value="ali">';
				    returnHtml += ' <label for="alipay"><img src="' + config.staticUrl + 'img/pay/alipay.png" width="150" height="45" alt="支付宝"></label>';
				    returnHtml += '</li>';
				}
				if (config.supportPayWay[i] == SurePayUtil.PAY_TYPE_WECHAT&&($.os.wechat||!$.os.mobile||$.os.iPad)) {
				    ...
				    returnHtml += ' <label for="wechat"><img src="' + config.staticUrl + 'img/pay/wechatpay.png" width="150" height="45" alt="微信在线支付"></label>';
				    returnHtml += '</li>';
				}
				if (config.supportPayWay[i] == SurePayUtil.PAY_TYPE_F2F) {
				    returnHtml += '<li>';
				    returnHtml += ' <input id="f2f" type="radio" name="formPayWayR"  value="f2f">';
				    returnHtml += ' <label for="f2f"><img src="' + config.staticUrl + 'img/pay/shoppay.png" width="150" height="45" alt="微信在线支付"></label>';
				    returnHtml += '</li>';
				}
			}
			returnHtml += '</ul>';
    ...
    var __PayApiObj = function(options) {
    	...
    	var defaultConfg = {
    		...
    		staticUrl : "/",
    		supportPayWay : [SurePayUtil.PAY_TYPE_ALI,  SurePayUtil.PAY_TYPE_WECHAT, SurePayUtil.PAY_TYPE_UPOP], //,
    		payFormId : "surepay_submit_form",
    		companyPhone : '400-9285-668',  //客服 电话  TODO 初始化从配置文件读
       ...
       this.initShowPayHtml = function (container, sCB,payWayChangeCB) {
            ...
            $("#" + po.config.payFormId + " input[name=formPayWayR]").on('ifClicked', function () {
				var payWay = $(this).val();
				if (payWay == SurePayUtil.PAY_TYPE_F2F)
					SureMsg.info("点击提交即可完成交易，无需在线支付，具体以商家店铺价格为准。");

                $("#" + po.config.payFormId + " input[name=payWay]").val($(this).val());
				if (typeof(payWayChangeCB) == "function") {
					payWayChangeCB($(this).val());
				}
