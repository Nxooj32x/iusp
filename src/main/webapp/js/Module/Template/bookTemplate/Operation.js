Ext.define('Module.Template.bookTemplate.Operation', {
	singleton: true, 
	...
	requires  : [
		...
		'Module.Template.bookTemplate.Tools'
	],
	...
	onAddSectionToBookClick : function(bookId, pos, stpl, success, failure){
		var form = Module.Template.bookTemplate.Tools.getCreateOrUpdateForm(stpl);
		var section = {};
		...
		form.down('button[name=submit]').on('click',function(btn){
			...
			Soul.Ajax.request({
           	   url : '/api/bps/' + bookId + '/section/?pos=' + pos,
           	   jsonData : section,
           	   headers : {
	 ...
	 * @param callback
	 */
	onModifySectionClick : function(bookId, section, callback) {
		var form = Module.Template.bookTemplate.Tools.getCreateOrUpdateForm(section.stpl);
		form.on("afterrender", function(f, eo){
			f.getForm().setValues(section);
			f.getForm().setValues({
		...
		form.down('button[name=submit]').on('click',function(btn){
			...
			if (values.showInCatalog)
				section.showInCatalog = values.showInCatalog;
			Soul.Ajax.request({
				url : "/api/bps/" + bookId + "/section/" + section.id ,
				method : 'put',
				jsonData  : section,
				loadMask : true,
				loadMsg : '修改中',
	...
	updateBookDataPoolClick : function(callback){
		Soul.Ajax.request({
			url : "/api/book/tpl/dataPool",
			method : 'put',
			params : {},
			jsonData : {},
			loadMask : true,
	...
	bookTplThumbnailUpdateClick : function(id,callback){
		Soul.Ajax.request({
			url : "/api/bookTpl/thumbnail",
			method : 'put',
			params : {tplId:id},
			jsonData : {},
			loadMask : true,
