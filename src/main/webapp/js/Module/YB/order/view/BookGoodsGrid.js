Ext.define('Module.YB.order.view.BookGoodsGrid', {
    ...
    initComponent: function () {
        ...
        Ext.apply(this, {
            ...
            columns: columns,
            store : {
                fields: ['name', 'status', 'price', 'binding', 'num', 'totalPrice', 'size', 'bookId', 'code']
            },
            viewConfig: {
    ...
    onPreviewClick : function(view ,rowIndex, colIndex, item, e, record, row){
        var me = this;
        var code  = me.order.code;
        var previewUrl = "/book/" + record.get("bookId") + "/preview?mode=turn&booktype=printBook&layout=small&orderCode=" + code;
        window.open(previewUrl);
    },
    
    onEditClick : function(view ,rowIndex, colIndex, item, e, record, row){
        var editUrl = "/book/" + record.get("bookId") + "/space";
        window.open(editUrl);
    },
